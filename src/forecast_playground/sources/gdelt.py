...
We scan a bounded window of the most recent 15-min slots at-or-before the as-of
instant and return matching article URLs. This is a breadth source (raw article
links across ~thousands of outlets), complementary to the curated
...
class GDELTNewsSource:
    ...
    Args:
        slots: How many 15-min GKG files to scan, working backward from the as-of
            instant (each ~6 MB / ~1500 articles). Keep small; the default covers
            ~3 hours of global news.
    ...
    def fetch(self, query: str, clock: Clock, **kwargs: Any) -> list[Document]:
        """Return article URLs matching ``query`` from GKG slots up to the Clock.
